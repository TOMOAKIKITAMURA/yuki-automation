...
def make_overlay(text, position, dst, style, width, height):
    ...
    fp = find_font()
    fs = int(style.get("font_size", 62))
    font = ImageFont.truetype(fp, fs) if fp else ImageFont.load_default()

    color = tuple(style.get("color", [246, 242, 234, 255]))
    shadow = tuple(style.get("shadow_color", [0, 0, 0, 110]))
    mx = int(style.get("margin_x", 82))
    my = int(style.get("margin_y", 300))
    gap = int(style.get("line_gap", 16))
...
def add_audio(video_path, output_path, cfg):
    audio_cfg = cfg.get("audio", {})
    enabled = bool(audio_cfg.get("enabled", False))
    audio_file = audio_cfg.get("file")

    if not enabled or not audio_file:
        run([
            "ffmpeg", "-y",
            "-i", str(video_path),
            "-c", "copy",
    ...
    volume = float(audio_cfg.get("volume", 0.16))
    fade_in = float(audio_cfg.get("fade_in", 0.5))
    fade_out = float(audio_cfg.get("fade_out", 1.2))

    total_duration = sum(float(c.get("duration", 2.0)) for c in cfg["clips"])
...
def main():
    ...
    ap.add_argument("--output", default="output/yuki_reel_v4_1.mp4")
    args = ap.parse_args()
